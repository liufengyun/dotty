...
class Analyzer(cls: ClassSymbol) { analyzer =>
  ...
  private def fixpoint(uses: Set[Type], pos: SourcePosition)(implicit ctx: Context): Set[Symbol] = {
    ...
    def resolve(tp: TermRef): Symbol =
      if (tp.symbol.is(Private) || tp.symbol.is(Final)) tp.symbol
      else tp.symbol.matchingMember(cls.typeRef).orElse(tp.symbol)
        ...
        if (seen.contains(head)) accumulate(tail, seen, acc)
          ...
          val acc2 = head match {
            case tmref: TermRef =>
              ...
              if (!sym.is(Method) && !sym.is(Lazy) && !sym.info.isInstanceOf[ConstantType]) {
